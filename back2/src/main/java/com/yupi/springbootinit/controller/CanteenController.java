...
import com.yupi.springbootinit.service.CanteenService;
...
public class CanteenController {
    ...
    @ApiOperation("分页查询温湿电能数据（食堂，支持多条件查询）")
    @PostMapping("/query")
    public BaseResponse<Page<TempMonitor>> queryByCondition(@RequestBody TempMonitorQueryRequest request) {
        ...
        Page<TempMonitor> page = canteenService.queryByCondition(request);
        return ResultUtils.success(page);
    }



    @ApiOperation("总电能消耗（食堂）")
    ...
    public BaseResponse<TempMonitorStatistics> getStatistics(
        ...
        try {
            TempMonitorStatistics statistics = canteenService.getStatistics("食堂", startTime, endTime);
    ...
    @ApiOperation("获取每小时电能消耗数据（食堂，查询模式-历史数据）")
    ...
    public BaseResponse<List<HourlyEnergyConsumption>> getHourlyEnergyConsumptionQuery(
        ...
        try {
            ...
            List<HourlyEnergyConsumption> consumptionData = canteenService.getHourlyEnergyConsumptionQuery(fixedWorkshop, deviceId, startTime, endTime);
    ...
    public BaseResponse<List<DailyEnergyConsumption>> getDailyEnergyConsumptionQuery(
        ...
        try {
            ...
            log.info("🟡 Controller调用日模式查询API: /electric-energy-daily-consumption-query - 车间: {}, 设备: {}, 开始时间: {}, 结束时间: {}",
                    fixedWorkshop, deviceId, startTime, endTime);
            List<DailyEnergyConsumption> consumptionData = canteenService.getDailyEnergyConsumptionQuery(fixedWorkshop, deviceId, startTime, endTime);
            return ResultUtils.success(consumptionData);
        } catch (Exception e) {
    ...
    public BaseResponse<Double> getEnergyConsumption(
        ...
        try {
            Double consumption = canteenService.getEnergyConsumption(startTime, endTime, mode);
    ...
    @ApiOperation("刷新缓存并预加载上个时间段数据（食堂）")
    @PostMapping("/refresh-cache")
    public BaseResponse<Boolean> refreshCache(
            @ApiParam("车间名称，可为空，默认食堂") @RequestParam(required = false) String workshop) {
        try {
            String fixedWorkshop = (workshop == null || workshop.isEmpty()) ? "食堂" : workshop;
