...
import com.yupi.springbootinit.service.Elevator114Service;
...
@Api(tags = "114_2#厂房电梯车间电能监控数据管理")
@RestController
@RequestMapping("/elevator114")
@Slf4j
public class Elevator114Controller {

    @Autowired
    private Elevator114Service elevator114Service;

    @Autowired
    private TempMonitorCacheLoader tempMonitorCacheLoader;


    @ApiOperation("分页查询温湿电能数据（114_2#厂房电梯，支持多条件查询）")
    @PostMapping("/query")
    public BaseResponse<Page<TempMonitor>> queryByCondition(@RequestBody TempMonitorQueryRequest request) {
        ...
        Page<TempMonitor> page = elevator114Service.queryByCondition(request);
        return ResultUtils.success(page);
    }



    @ApiOperation("总电能消耗（114_2#厂房电梯）")
    ...
    public BaseResponse<TempMonitorStatistics> getStatistics(
        ...
        try {
            TempMonitorStatistics statistics = elevator114Service.getStatistics("114_2#厂房电梯", startTime, endTime);
    ...
    @ApiOperation("获取每小时电能消耗数据（114_2#厂房电梯，查询模式-历史数据）")
    ...
    public BaseResponse<List<HourlyEnergyConsumption>> getHourlyEnergyConsumptionQuery(
        ...
        try {
            ...
            String fixedWorkshop = "114_2#厂房电梯";
            ...
            List<HourlyEnergyConsumption> consumptionData = elevator114Service.getHourlyEnergyConsumptionQuery(fixedWorkshop, deviceId, startTime, endTime);
    ...
    public BaseResponse<List<DailyEnergyConsumption>> getDailyEnergyConsumptionQuery(
        ...
        try {
            ...
            log.info("🟡 Controller调用日模式查询API: /electric-energy-daily-consumption-query - 车间: {}, 设备: {}, 开始时间: {}, 结束时间: {}",
                    fixedWorkshop, deviceId, startTime, endTime);
            List<DailyEnergyConsumption> consumptionData = elevator114Service.getDailyEnergyConsumptionQuery(fixedWorkshop, deviceId, startTime, endTime);
            return ResultUtils.success(consumptionData);
        } catch (Exception e) {
    ...
    @ApiOperation("电能消耗卡片（114_2#厂房电梯，后端计算差值）")
    ...
    public BaseResponse<Double> getEnergyConsumption(
        ...
        try {
            Double consumption = elevator114Service.getEnergyConsumption(startTime, endTime, mode);
    ...
    @ApiOperation("刷新缓存并预加载上个时间段数据（114_2#厂房电梯）")
    @PostMapping("/refresh-cache")
    public BaseResponse<Boolean> refreshCache(
            @ApiParam("车间名称，可为空，默认114_2#厂房电梯") @RequestParam(required = false) String workshop) {
        try {
            String fixedWorkshop = (workshop == null || workshop.isEmpty()) ? "114_2#厂房电梯" : workshop;
