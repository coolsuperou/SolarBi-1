...
public class Granule102Controller {
    ...
    @ApiOperation("分页查询温湿电能数据（102造粒，支持多条件查询）")
    @PostMapping("/query")
    ...
    public BaseResponse<TempMonitorStatistics> getStatistics(
        ...
        try {
            TempMonitorStatistics statistics = granule102Service.getStatistics("102造粒", startTime, endTime);
    ...
    @ApiOperation("获取每小时电能消耗数据（102造粒，查询模式-历史数据）")
    ...
    @ApiOperation("电能消耗卡片（102造粒，后端计算差值）")
    ...
    public BaseResponse<Double> getEnergyConsumption(
        ...
        try {
            Double consumption = granule102Service.getEnergyConsumption(startTime, endTime, mode);
    ...
    @ApiOperation("刷新缓存并预加载上个时间段数据（102造粒）")
