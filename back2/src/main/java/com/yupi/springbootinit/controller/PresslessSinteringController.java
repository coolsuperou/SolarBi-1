...
@Api(tags = "无压烧结车间电能监控数据管理")
@RestController
@RequestMapping("/pressless-sintering")
@Slf4j
public class PresslessSinteringController {

    @Autowired
    private PresslessSinteringService presslessSinteringService;

    @Autowired
    private TempMonitorCacheLoader tempMonitorCacheLoader;

    @ApiOperation("分页查询温湿电能数据（无压烧结，支持多条件查询）")
    @PostMapping("/query")
    public BaseResponse<Page<TempMonitor>> queryByCondition(@RequestBody TempMonitorQueryRequest request) {
        ...
        Page<TempMonitor> page = presslessSinteringService.queryByCondition(request);
        return ResultUtils.success(page);
    }

    @ApiOperation("总电能消耗（无压烧结）")
    ...
    public BaseResponse<TempMonitorStatistics> getStatistics(
        ...
        try {
            TempMonitorStatistics statistics = presslessSinteringService.getStatistics("无压烧结", startTime, endTime);
    ...
    public BaseResponse<List<HourlyEnergyConsumption>> getHourlyEnergyConsumptionQuery(
        ...
        try {
            ...
            List<HourlyEnergyConsumption> consumptionData = presslessSinteringService.getHourlyEnergyConsumptionQuery(fixedWorkshop, deviceId, startTime, endTime);
    ...
    public BaseResponse<List<DailyEnergyConsumption>> getDailyEnergyConsumptionQuery(
        ...
        try {
            ...
            log.info("🟡 Controller调用日模式查询API: /electric-energy-daily-consumption-query - 车间: {}, 设备: {}, 开始时间: {}, 结束时间: {}",
                    fixedWorkshop, deviceId, startTime, endTime);
            List<DailyEnergyConsumption> consumptionData = presslessSinteringService.getDailyEnergyConsumptionQuery(fixedWorkshop, deviceId, startTime, endTime);
            return ResultUtils.success(consumptionData);
        } catch (Exception e) {
    ...
    @ApiOperation("电能消耗卡片（无压烧结，后端计算差值）")
    ...
    public BaseResponse<Double> getEnergyConsumption(
        ...
        try {
            Double consumption = presslessSinteringService.getEnergyConsumption(startTime, endTime, mode);
    ...
    @ApiOperation("刷新缓存并预加载上个时间段数据（无压烧结）")
    @PostMapping("/refresh-cache")
    public BaseResponse<Boolean> refreshCache(
            @ApiParam("车间名称，可为空，默认无压烧结") @RequestParam(required = false) String workshop) {
        try {
            String fixedWorkshop = (workshop == null || workshop.isEmpty()) ? "无压烧结" : workshop;
