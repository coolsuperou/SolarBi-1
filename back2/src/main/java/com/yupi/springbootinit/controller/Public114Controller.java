...
import com.yupi.springbootinit.service.Public114Service;
...
@Api(tags = "114公共车间电能监控数据管理")
@RestController
@RequestMapping("/public114")
@Slf4j
public class Public114Controller {

    @Autowired
    private Public114Service public114Service;

    @Autowired
    private TempMonitorCacheLoader tempMonitorCacheLoader;


    @ApiOperation("分页查询温湿电能数据（114公共，支持多条件查询）")
    @PostMapping("/query")
    public BaseResponse<Page<TempMonitor>> queryByCondition(@RequestBody TempMonitorQueryRequest request) {
        ...
        Page<TempMonitor> page = public114Service.queryByCondition(request);
        return ResultUtils.success(page);
    }



    @ApiOperation("总电能消耗（114公共）")
    ...
    public BaseResponse<TempMonitorStatistics> getStatistics(
        ...
        try {
            TempMonitorStatistics statistics = public114Service.getStatistics("114公共", startTime, endTime);
    ...
    @ApiOperation("获取每小时电能消耗数据（114公共，查询模式-历史数据）")
    ...
    public BaseResponse<List<HourlyEnergyConsumption>> getHourlyEnergyConsumptionQuery(
        ...
        try {
            ...
            List<HourlyEnergyConsumption> consumptionData = public114Service.getHourlyEnergyConsumptionQuery(fixedWorkshop, deviceId, startTime, endTime);
    ...
    public BaseResponse<List<DailyEnergyConsumption>> getDailyEnergyConsumptionQuery(
        ...
        try {
            ...
            log.info("🟡 Controller调用日模式查询API: /electric-energy-daily-consumption-query - 车间: {}, 设备: {}, 开始时间: {}, 结束时间: {}",
                    fixedWorkshop, deviceId, startTime, endTime);
            List<DailyEnergyConsumption> consumptionData = public114Service.getDailyEnergyConsumptionQuery(fixedWorkshop, deviceId, startTime, endTime);
            return ResultUtils.success(consumptionData);
        } catch (Exception e) {
    ...
    @ApiOperation("电能消耗卡片（114公共，后端计算差值）")
    ...
    public BaseResponse<Double> getEnergyConsumption(
        ...
        try {
            Double consumption = public114Service.getEnergyConsumption(startTime, endTime, mode);
    ...
    @ApiOperation("刷新缓存并预加载上个时间段数据（114公共）")
    @PostMapping("/refresh-cache")
    public BaseResponse<Boolean> refreshCache(
            @ApiParam("车间名称，可为空，默认114公共") @RequestParam(required = false) String workshop) {
        try {
            String fixedWorkshop = (workshop == null || workshop.isEmpty()) ? "114公共" : workshop;
