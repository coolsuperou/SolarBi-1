...
@Api(tags = "109炼胶车间电能监控数据管理")
@RestController
@RequestMapping("/rubber-109")
@Slf4j
public class Rubber109Controller {

    @Autowired
    private Rubber109Service rubber109Service;

    @Autowired
    private TempMonitorCacheLoader tempMonitorCacheLoader;


    @ApiOperation("分页查询温湿电能数据（109炼胶，支持多条件查询）")
    @PostMapping("/query")
    public BaseResponse<Page<TempMonitor>> queryByCondition(@RequestBody TempMonitorQueryRequest request) {
        ...
        Page<TempMonitor> page = rubber109Service.queryByCondition(request);
        return ResultUtils.success(page);
    }



    @ApiOperation("总电能消耗（109炼胶）")
    ...
    public BaseResponse<TempMonitorStatistics> getStatistics(
        ...
        try {
            TempMonitorStatistics statistics = rubber109Service.getStatistics("109炼胶", startTime, endTime);
    ...
    @ApiOperation("获取每小时电能消耗数据（109炼胶，查询模式-历史数据）")
    ...
    public BaseResponse<List<HourlyEnergyConsumption>> getHourlyEnergyConsumptionQuery(
        ...
        try {
            ...
            List<HourlyEnergyConsumption> consumptionData = rubber109Service.getHourlyEnergyConsumptionQuery(fixedWorkshop, deviceId, startTime, endTime);
    ...
    public BaseResponse<List<DailyEnergyConsumption>> getDailyEnergyConsumptionQuery(
        ...
        try {
            ...
            log.info("🟡 Controller调用日模式查询API: /electric-energy-daily-consumption-query - 车间: {}, 设备: {}, 开始时间: {}, 结束时间: {}",
                    fixedWorkshop, deviceId, startTime, endTime);
            List<DailyEnergyConsumption> consumptionData = rubber109Service.getDailyEnergyConsumptionQuery(fixedWorkshop, deviceId, startTime, endTime);
            return ResultUtils.success(consumptionData);
        } catch (Exception e) {
    ...
    @ApiOperation("电能消耗卡片（109炼胶，后端计算差值）")
    ...
    public BaseResponse<Double> getEnergyConsumption(
        ...
        try {
            Double consumption = rubber109Service.getEnergyConsumption(startTime, endTime, mode);
    ...
    @ApiOperation("刷新缓存并预加载上个时间段数据（109炼胶）")
    @PostMapping("/refresh-cache")
    public BaseResponse<Boolean> refreshCache(
            @ApiParam("车间名称，可为空，默认109炼胶") @RequestParam(required = false) String workshop) {
        try {
            String fixedWorkshop = (workshop == null || workshop.isEmpty()) ? "109炼胶" : workshop;
