 ...
 */
@Mapper
public interface FeedingWorkshopMapper extends BaseMapper<TempMonitor> {

    /**
     * 查询最新数据（显示所有数据）
     */
    @Select("SELECT Id, DeviceID, Name, Tem, Hum, MAC, UpdateTime, ElectricEnergy, NodeID, Workshop " +
            "FROM RSWS_TempMonitor_Copy WHERE Workshop = '101配料' ORDER BY UpdateTime DESC")
     ...
     * 查询所有车间
     */
    @Select("SELECT DISTINCT Workshop FROM RSWS_TempMonitor_Copy WHERE Workshop = '101配料'")
