...
/**
 * 110注射环保设备温湿电能监控数据 Mapper
 * 使用 SQL Server 数据源
 * 
 * @author yupi
 */
@Mapper
public interface InjectionWorkshopMapper extends BaseMapper<TempMonitor> {

    /**
     * 查询最新数据（显示所有数据）
     */
    @Select("SELECT Id, DeviceID, Name, Tem, Hum, MAC, UpdateTime, ElectricEnergy, NodeID, Workshop " +
            "FROM RSWS_TempMonitor_Copy WHERE Workshop = '110注射环保设备' ORDER BY UpdateTime DESC")
     ...
     * 查询所有车间
     */
    @Select("SELECT DISTINCT Workshop FROM RSWS_TempMonitor_Copy WHERE Workshop = '110注射环保设备'")
     ...
     */
    @Select("SELECT Id, DeviceID, Name, Tem, Hum, MAC, UpdateTime, ElectricEnergy, NodeID, Workshop " +
            "FROM RSWS_TempMonitor_Copy WHERE DeviceID = #{deviceId} AND Workshop = '110注射环保设备' ORDER BY UpdateTime DESC")
