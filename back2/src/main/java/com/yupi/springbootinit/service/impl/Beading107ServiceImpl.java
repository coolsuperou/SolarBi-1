...
public class Beading107ServiceImpl implements Beading107Service {
    ...
    public List<HourlyEnergyConsumption> getHourlyEnergyConsumptionQuery(String workshop, String deviceId, Date startTime, Date endTime) {
        ...
        List<HourlyEnergyConsumption> result = beading107Mapper.selectHourlyEnergyConsumptionQuery(fixedWorkshop, deviceId, startTime, endTime);
        
        // 添加调试日志，输出实际查询结果
        if (result != null && !result.isEmpty()) {
            log.info("=== 每小时电能消耗数据调试信息（查询模式-107串珠） ===");
    ...
    public List<DailyEnergyConsumption> getDailyEnergyConsumptionQuery(String workshop, String deviceId, Date startTime, Date endTime) {
        ...
        List<DailyEnergyConsumption> result = beading107Mapper.selectDailyEnergyConsumptionQuery(fixedWorkshop, deviceId, startTime, endTime);
        
        // 添加调试日志，输出实际查询结果
        if (result != null && !result.isEmpty()) {
            log.info("=== 每日电能消耗数据调试信息（查询模式-107串珠） ===");
    ...
    public Double getEnergyConsumption(Date startTime, Date endTime, String mode) {
        ...
        Double consumption = beading107Mapper.selectEnergyConsumption(startTime, endTime, mode);
