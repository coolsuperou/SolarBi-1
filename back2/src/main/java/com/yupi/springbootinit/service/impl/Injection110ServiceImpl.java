...
import com.yupi.springbootinit.mapper.sqlserver.Injection110Mapper;
...
@Service
@Slf4j
public class Injection110ServiceImpl implements Injection110Service {

    @Autowired
    private Injection110Mapper injection110Mapper;
    ...
    public Page<TempMonitor> queryByCondition(TempMonitorQueryRequest request) {
        ...
        return injection110Mapper.selectPageByCondition(
            page,
            request.getDeviceId(),
            request.getName(),
            fixedWorkshop,
    ...
    public TempMonitorStatistics getStatistics(String workshop, Date startTime, Date endTime) {
        // 服务层也强制限定车间
        String fixedWorkshop = "110注射";
        TempMonitorStatistics statistics = injection110Mapper.getStatistics(fixedWorkshop, startTime, endTime);
    ...
    public List<HourlyEnergyConsumption> getHourlyEnergyConsumptionQuery(String workshop, String deviceId, Date startTime, Date endTime) {
        ...
        List<HourlyEnergyConsumption> result = injection110Mapper.selectHourlyEnergyConsumptionQuery(fixedWorkshop, deviceId, startTime, endTime);
        
        // 添加调试日志，输出实际查询结果
        if (result != null && !result.isEmpty()) {
            log.info("=== 每小时电能消耗数据调试信息（查询模式-110注射） ===");
    ...
    public List<DailyEnergyConsumption> getDailyEnergyConsumptionQuery(String workshop, String deviceId, Date startTime, Date endTime) {
        ...
        List<DailyEnergyConsumption> result = injection110Mapper.selectDailyEnergyConsumptionQuery(fixedWorkshop, deviceId, startTime, endTime);
        
        // 添加调试日志，输出实际查询结果
        if (result != null && !result.isEmpty()) {
            log.info("=== 每日电能消耗数据调试信息（查询模式-110注射） ===");
    ...
    public Double getEnergyConsumption(Date startTime, Date endTime, String mode) {
        ...
        Double consumption = injection110Mapper.selectEnergyConsumption(startTime, endTime, mode);
