...
import com.yupi.springbootinit.mapper.sqlserver.InjectionWorkshopMapper;
...
@Service
@Slf4j
public class InjectionWorkshopServiceImpl implements InjectionWorkshopService {

    @Autowired
    private InjectionWorkshopMapper injectionWorkshopMapper;
    ...
    public List<TempMonitor> getLatestData() {
        ...
        return injectionWorkshopMapper.selectByWorkshop(fixedWorkshop);
    }
    ...
    public Page<TempMonitor> queryByCondition(TempMonitorQueryRequest request) {
        ...
        // 调用Mapper进行查询
        return injectionWorkshopMapper.selectPageByCondition(
            page,
            request.getDeviceId(),
            request.getName(),
            fixedWorkshop,
    ...
    public TempMonitorStatistics getStatistics(String workshop, Date startTime, Date endTime) {
        // 服务层也强制限定车间
        String fixedWorkshop = "110注射环保设备";
        TempMonitorStatistics statistics = injectionWorkshopMapper.getStatistics(fixedWorkshop, startTime, endTime);
        ...
        // 获取设备数量
        Integer deviceCount = injectionWorkshopMapper.countDevices(fixedWorkshop, startTime, endTime);
    ...
    public Integer getDeviceCount(String workshop, Date startTime, Date endTime) {
        // 服务层也强制限定车间
        String fixedWorkshop = "110注射环保设备";
        Integer count = injectionWorkshopMapper.countDevices(fixedWorkshop, startTime, endTime);
    ...
    public List<TempMonitor> getElectricEnergyTrend(String workshop, String deviceId, Date startTime, Date endTime, Integer limit) {
        ...
        return injectionWorkshopMapper.selectElectricEnergyTrend(fixedWorkshop, deviceId, startTime, endTime, limit);
    }
    ...
    public List<HourlyEnergyConsumption> getHourlyEnergyConsumption(String workshop, String deviceId, Date startTime, Date endTime) {
        ...
        List<HourlyEnergyConsumption> result = injectionWorkshopMapper.selectHourlyEnergyConsumption(fixedWorkshop, deviceId, startTime, endTime);
        
        // 添加调试日志，输出实际查询结果
        if (result != null && !result.isEmpty()) {
            log.info("=== 每小时电能消耗数据调试信息（默认模式-110注射环保设备） ===");
    ...
    public List<HourlyEnergyConsumption> getHourlyEnergyConsumptionQuery(String workshop, String deviceId, Date startTime, Date endTime) {
        ...
        List<HourlyEnergyConsumption> result = injectionWorkshopMapper.selectHourlyEnergyConsumptionQuery(fixedWorkshop, deviceId, startTime, endTime);
        
        // 添加调试日志，输出实际查询结果
        if (result != null && !result.isEmpty()) {
            log.info("=== 每小时电能消耗数据调试信息（查询模式-110注射环保设备） ===");
            for (HourlyEnergyConsumption item : result) {
                log.info("设备: {}, 小时: {}, 开始能耗: {}, 结束能耗: {}, 消耗量: {}, 开始时间: {}, 结束时间: {}", 
                    item.getDeviceId(), 
                    item.getHour(), 
    ...
    public List<DailyEnergyConsumption> getDailyEnergyConsumptionQuery(String workshop, String deviceId, Date startTime, Date endTime) {
        ...
        List<DailyEnergyConsumption> result = injectionWorkshopMapper.selectDailyEnergyConsumptionQuery(fixedWorkshop, deviceId, startTime, endTime);
        
        // 添加调试日志，输出实际查询结果
        if (result != null && !result.isEmpty()) {
            log.info("=== 每日电能消耗数据调试信息（查询模式-110注射环保设备） ===");
