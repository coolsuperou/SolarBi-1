...
import com.yupi.springbootinit.mapper.sqlserver.Laboratory114Mapper;
...
@Service
@Slf4j
public class Laboratory114ServiceImpl implements Laboratory114Service {

    @Autowired
    private Laboratory114Mapper laboratory114Mapper;
    ...
    public Page<TempMonitor> queryByCondition(TempMonitorQueryRequest request) {
        ...
        return laboratory114Mapper.selectPageByCondition(
            page,
            request.getDeviceId(),
            request.getName(),
            fixedWorkshop,
    ...
    public TempMonitorStatistics getStatistics(String workshop, Date startTime, Date endTime) {
        // 服务层也强制限定车间
        String fixedWorkshop = "114_2#楼实验室";
        TempMonitorStatistics statistics = laboratory114Mapper.getStatistics(fixedWorkshop, startTime, endTime);
...
public List<HourlyEnergyConsumption> getHourlyEnergyConsumptionQuery(String workshop, String deviceId, Date startTime, Date endTime) {
    ...
    // 🔥 使用现有的非分页查询方法
    List<TempMonitor> rawData = laboratory114Mapper.selectHourlyRawData(
    ...
    if (result != null && !result.isEmpty()) {
        log.info("=== 每小时电能消耗数据调试信息（优化版本-114_2#楼实验室） ===");
    ...
    public List<DailyEnergyConsumption> getDailyEnergyConsumptionQuery(String workshop, String deviceId, Date startTime, Date endTime) {
        ...
        // 保持原有调试日志
        if (result != null && !result.isEmpty()) {
            log.info("=== 每日电能消耗数据调试信息（优化版本-114_2#楼实验室） ===");
