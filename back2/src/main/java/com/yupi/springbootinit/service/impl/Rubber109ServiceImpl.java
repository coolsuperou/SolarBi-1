...
import com.yupi.springbootinit.mapper.sqlserver.Rubber109Mapper;
...
@Service
@Slf4j
public class Rubber109ServiceImpl implements Rubber109Service {

    @Autowired
    private Rubber109Mapper rubber109Mapper;
    ...
    public Page<TempMonitor> queryByCondition(TempMonitorQueryRequest request) {
        ...
        return rubber109Mapper.selectPageByCondition(
            page,
            request.getDeviceId(),
            request.getName(),
            fixedWorkshop,
    ...
    public TempMonitorStatistics getStatistics(String workshop, Date startTime, Date endTime) {
        // 服务层也强制限定车间
        String fixedWorkshop = "109炼胶";
        TempMonitorStatistics statistics = rubber109Mapper.getStatistics(fixedWorkshop, startTime, endTime);
    ...
    public List<HourlyEnergyConsumption> getHourlyEnergyConsumptionQuery(String workshop, String deviceId, Date startTime, Date endTime) {
        ...
        List<HourlyEnergyConsumption> result = rubber109Mapper.selectHourlyEnergyConsumptionQuery(fixedWorkshop, deviceId, startTime, endTime);
        
        // 添加调试日志，输出实际查询结果
        if (result != null && !result.isEmpty()) {
            log.info("=== 每小时电能消耗数据调试信息（查询模式-109炼胶） ===");
    ...
    public List<DailyEnergyConsumption> getDailyEnergyConsumptionQuery(String workshop, String deviceId, Date startTime, Date endTime) {
        ...
        List<DailyEnergyConsumption> result = rubber109Mapper.selectDailyEnergyConsumptionQuery(fixedWorkshop, deviceId, startTime, endTime);
        
        // 添加调试日志，输出实际查询结果
        if (result != null && !result.isEmpty()) {
            log.info("=== 每日电能消耗数据调试信息（查询模式-109炼胶） ===");
    ...
    public Double getEnergyConsumption(Date startTime, Date endTime, String mode) {
        ...
        Double consumption = rubber109Mapper.selectEnergyConsumption(startTime, endTime, mode);
