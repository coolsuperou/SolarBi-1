...
import com.yupi.springbootinit.mapper.sqlserver.Sintering105Mapper;
...
@Service
@Slf4j
public class Sintering105ServiceImpl implements Sintering105Service {

    @Autowired
    private Sintering105Mapper sintering105Mapper;
    ...
    public Page<TempMonitor> queryByCondition(TempMonitorQueryRequest request) {
        ...
        String fixedWorkshop = "105烧结";
        
        return sintering105Mapper.selectPageByCondition(
            page,
            request.getDeviceId(),
            request.getName(),
            fixedWorkshop,
    ...
    public TempMonitorStatistics getStatistics(String workshop, Date startTime, Date endTime) {
        // 服务层也强制限定车间
        String fixedWorkshop = "105烧结";
        TempMonitorStatistics statistics = sintering105Mapper.getStatistics(fixedWorkshop, startTime, endTime);
    ...
    public List<HourlyEnergyConsumption> getHourlyEnergyConsumptionQuery(String workshop, String deviceId, Date startTime, Date endTime) {
        ...
        List<HourlyEnergyConsumption> result = sintering105Mapper.selectHourlyEnergyConsumptionQuery(fixedWorkshop, deviceId, startTime, endTime);
        
        // 添加调试日志，输出实际查询结果
        if (result != null && !result.isEmpty()) {
            log.info("=== 每小时电能消耗数据调试信息（查询模式-105烧结） ===");
    ...
    public List<DailyEnergyConsumption> getDailyEnergyConsumptionQuery(String workshop, String deviceId, Date startTime, Date endTime) {
        ...
        List<DailyEnergyConsumption> result = sintering105Mapper.selectDailyEnergyConsumptionQuery(fixedWorkshop, deviceId, startTime, endTime);
        
        // 添加调试日志，输出实际查询结果
        if (result != null && !result.isEmpty()) {
            log.info("=== 每日电能消耗数据调试信息（查询模式-105烧结） ===");
    ...
    public Double getEnergyConsumption(Date startTime, Date endTime, String mode) {
        ...
        Double consumption = sintering105Mapper.selectEnergyConsumption(startTime, endTime, mode);
